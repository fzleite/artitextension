...
chrome.runtime.onMessage.addListener( ( message, sender, sendResponse ) => {
    ...
    // Recebe mensagem de disconexão
        ...
        console.log( app.firebase.userDB + "/conectado" );

        let dbLanc = firebase.database().ref( app.firebase.userDB + "/conectado");
        dbLanc.set("Nao");
...
function checkUser(){
    ...
    if( userName !== null )
        app.firebase.userDB = app.firebase.mainDB + userName.split("@")[0];
...
function runRosieFirebase(){
    ...
    try{
        ...
        if( user !== null ){
            ...
            let dbLanc = firebase.database().ref( app.firebase.userDB );
            ...
            dbLanc.once('value', fbData => {
                ...
                if( typeof userData !== 'undefined' ){
                    ...
                    // Trata o ultimo apontamento
                    if( typeof lastRegister === 'undefined' ){
                        lastRegister = localStorage.getItem("lastCheck");
                    ...
                    if( typeof userData.mensagem !== 'undefined' ){
                        let count = userData.viewCount;
                        let limite = userData.limiteExibicao;

                        limite = ( typeof userData.limiteExibicao === 'undefined' ? 10 : userData.limiteExibicao );
                        ...
                        if( count < limite ){
                            ...
                            userData.ultima_notificacao = typeof userData.ultima_notificacao === 'undefined' ?
                                null : 
                                userData.ultima_notificacao;

                            userData.viewCount = count;
                            userData.limiteExibicao = limite;
                        }
                    }
                    dbLanc.set(userData);
...
function sendNotification( notificationArgs, sendImage ) {
    ...
    try{
        ...
        if (Notification.permission !== "granted")
            ...
            let dbLastNotif = firebase.database().ref( app.firebase.userDB + "/ultima_notificacao");
            let ultima_notificacao = getCurrentDate();
            dbLastNotif.set( ultima_notificacao );
            ...
            chrome.notifications.onButtonClicked.addListener( (id, index) => {
                if( id === bg.alarm.notificationId ){
                    ...
                    if( index === 0 ){
                        ...
                        let dbApontamento = firebase.database().ref( app.firebase.userDB );
                        dbApontamento.once('value', snap => {
                            ...
                            dbApontamento.set({
                                ...
                                ultimo_apontamento : userData.ultimo_apontamento,
                                ultima_notificacao : ultima_notificacao,
                            });
                            console.log("Solicitada exclusao");    
                        })
